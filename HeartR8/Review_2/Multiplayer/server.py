...
from Multiplayer import server_game
...
def main():
    ...
    print('Player-Name: ', name_list)  # выводим список имен игроков вместе с соответствующим номером игрока
    names = [None] * number_of_players

    for i in name_list:  # записываем никнеймы в новый список в правильном порядке
        names[i[0]] = i[1]

    print('Names in the right order: ', names)

    field_size = 10  # размер игрового поля

    game = server_game.MyGame(field_size, player_list, names, server_players_list)  # инициализируем игровую сессию
    game.setup()  # запускаем игровую сессию
